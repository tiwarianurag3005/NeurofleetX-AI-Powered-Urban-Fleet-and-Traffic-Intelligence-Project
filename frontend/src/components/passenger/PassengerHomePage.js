...
const PassengerHomePage = ({ setPage, setCurrentRide, rides }) => {
    ...
    useEffect(() => {
        const destinationCounts = rides.reduce((acc, ride) => {
            if (ride.drop) { acc[ride.drop] = (acc[ride.drop] || 0) + 1; } return acc;
        }, {});
        const sortedDestinations = Object.entries(destinationCounts).sort((a, b) => b[1] - a[1]).map(item => item[0]).slice(0, 3);
        ...
        if (pickup && drop) {
            ...
            const generatedRoutes = [
                ...
                return { 
                    ...
                    time: `${mlEta} min`,
                    // Attach the passenger's requirements to the route
                    vehicleType,
                    passengerCount,
                };
            });
            ...
            setBookingMessage(isMatchingVehicleAvailable ? 'Please select a route to book.' : 'No vehicles match your criteria. See available vehicles below.');
            setPickupCoords({ x: '100px', y: '450px' });
            setDropCoords({ x: '800px', y: '150px' });
            setSelectedRoute(null); // Deselect route when new ones are found

        } else {
            setRoutes([]); setSelectedRoute(null); setPickupCoords(null); setDropCoords(null);
        }
    };
    ...
    }, [pickup, drop, vehicleType, passengerCount, isMatchingVehicleAvailable]);
    ...
    const getStatusColorClass = (status) => {
        if (status === 'All Clear') return 'border-teal-400'; if (status === 'Busy') return 'border-yellow-400'; if (status === 'Much Busy') return 'border-red-400'; return 'border-gray-600';
    };

    const handleSuggestionClick = (destination) => { setDrop(destination); if(!pickup) setPickup("City Center"); }
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div className="lg:col-span-1 space-y-6">
                     ...
                     {suggestedDestinations.length > 0 && (
                        <div className="space-y-2">
                             ...
                             <div className="flex flex-wrap gap-2">
                                {suggestedDestinations.map(dest => (<button key={dest} onClick={() => handleSuggestionClick(dest)} className="bg-gray-700 hover:bg-cyan-800 text-xs text-cyan-300 font-semibold py-1 px-3 rounded-full transition">{dest}</button>))}
                             </div>
                        </div>
                     )}
                     <div className="relative">
                        <MapPinIcon className="w-5 h-5 text-green-400 absolute top-1/2 left-3 -translate-y-1/2" />
                        <input type="text" value={pickup} onChange={(e) => setPickup(e.target.value)} placeholder="Enter Pickup Location" className="w-full bg-gray-700 border-gray-600 rounded-lg py-3 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                    </div>
                    <div className="relative">
                        <MapPinIcon className="w-5 h-5 text-red-400 absolute top-1/2 left-3 -translate-y-1/2" />
                        <input type="text" value={drop} onChange={(e) => setDrop(e.target.value)} placeholder="Enter Drop Location" className="w-full bg-gray-700 border-gray-600 rounded-lg py-3 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-cyan-500" />
                    </div>
                ...
                {routes.length > 0 && isMatchingVehicleAvailable && (
                    <div>
                        ...
                        <div className="space-y-3">
                            {routes.map(route => (
                                <button key={route.id} onClick={() => setSelectedRoute(route)} className={`w-full text-left p-4 rounded-lg border-2 transition ${selectedRoute?.id === route.id ? 'bg-cyan-900/50 border-cyan-400' : 'bg-gray-800 hover:bg-gray-700 ' + getStatusColorClass(route.status)}`}>
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold">{route.name}</span>
                                        <span className={`font-semibold text-sm ${getStatusColorClass(route.status).replace('border-','text-')}`}>{route.status}</span>
                                    </div>
                 ...
                 <div className="pt-4">
                    <button onClick={handleBookRide} disabled={!pickup || !drop || !selectedRoute || !isMatchingVehicleAvailable} className="w-full bg-cyan-600 hover:bg-cyan-700 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-bold py-4 rounded-lg transition duration-300 flex items-center justify-center space-x-2">
                        <span>Book Ride</span>
                ...
                {/* --- Display available vehicles if no match is found --- */}
                {routes.length > 0 && !isMatchingVehicleAvailable && (
             ...
             <div className="lg:col-span-2">
                <MapView vehiclePositions={availableVehicles} routes={routes} selectedRouteId={selectedRoute?.id} pickupCoords={pickupCoords} dropCoords={dropCoords} showHeatmap={showHeatmap} />
             </div>
        </div>
    );
};
